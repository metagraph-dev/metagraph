...
from metagraph.plugins.scipy.types import ScipySparseMatrixType
import numpy as np
import grblas
...
def test_numpy(default_plugin_resolver):
    ...
    x = NumpySparseMatrix(mat, missing_value=3)
    assert x.nnz == 4
    assert x.shape == (3, 3)
    # Convert back and forth from scipy.sparse
    ssm = dpr.translate(x, ss.spmatrix)
    assert isinstance(ssm, ss.spmatrix)
    y = dpr.translate(ssm, NumpySparseMatrix)
    assert isinstance(y, NumpySparseMatrix)
...
def test_scipy(default_plugin_resolver):
    ...
    npm = dpr.translate(x, NumpySparseMatrix)
    assert isinstance(npm, NumpySparseMatrix)
    y = dpr.translate(npm, ScipySparseMatrixType)
