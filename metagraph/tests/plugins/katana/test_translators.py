...
def test_topology(kg_from_nx_di_8_12):
    ...
    assert [
        kg_from_nx_di_8_12.value.get_edge_dest(i)
        for i in kg_from_nx_di_8_12.value.edge_ids(0)
    ] == [1, 3, 4]
    assert [
        kg_from_nx_di_8_12.value.get_edge_dest(i)
        for i in kg_from_nx_di_8_12.value.edge_ids(2)
    ] == [4, 5, 6]
    assert [
        kg_from_nx_di_8_12.value.get_edge_dest(i)
        for i in kg_from_nx_di_8_12.value.edge_ids(4)
    ] == [7]
    assert [
        kg_from_nx_di_8_12.value.get_edge_dest(i)
        for i in kg_from_nx_di_8_12.value.edge_ids(5)
    ] == [6, 7]
...
def test_compare_node_count(nx_from_kg_di_8_12, katanagraph_cleaned_8_12_di):
    nlist = [
        each_node[0] for each_node in list(nx_from_kg_di_8_12.value.nodes(data=True))
    ]
    num_no_edge_nodes = 0
    for nid in katanagraph_cleaned_8_12_di.value:
        if nid not in nlist:
            ...
            num_no_edge_nodes += 1
    assert (
        num_no_edge_nodes + len(nlist) == katanagraph_cleaned_8_12_di.value.num_nodes()
    )
...
def test_compare_edge_count(nx_from_kg_di_8_12, katanagraph_cleaned_8_12_di):
    edge_dict_count = {
        (each_e[0], each_e[1]): 0
        for each_e in list(nx_from_kg_di_8_12.value.edges(data=True))
    }
    for src in katanagraph_cleaned_8_12_di.value:
        for dest in [
            katanagraph_cleaned_8_12_di.value.get_edge_dest(e)
            for e in katanagraph_cleaned_8_12_di.value.edge_ids(src)
        ]:
            if (src, dest) in edge_dict_count:
                edge_dict_count[(src, dest)] += 1
    assert (
        sum([edge_dict_count[i] for i in edge_dict_count])
        == katanagraph_cleaned_8_12_di.value.num_edges()
    )
    assert (
        len(list(nx_from_kg_di_8_12.value.edges(data=True)))
        == katanagraph_cleaned_8_12_di.value.num_edges()
    )
