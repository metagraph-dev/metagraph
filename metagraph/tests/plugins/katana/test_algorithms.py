...
def test_sssp_bellman_ford(networkx_weighted_directed_8_12, kg_from_nx_di_8_12):
    src_node = 0
    sssp_nx = mg.algos.traversal.bellman_ford(
        networkx_weighted_directed_8_12, src_node
    )  # source node is 0
    ...
    parents_kg, distances_kg = mg.algos.traversal.bellman_ford(
        kg_from_nx_di_8_12, src_node
    )
...
def test_sssp_dijkstra(networkx_weighted_directed_8_12, kg_from_nx_di_8_12):
    src_node = 1
    sssp_nx = mg.algos.traversal.dijkstra(
        networkx_weighted_directed_8_12, src_node
    )  # source node is 1
    parents_nx = sssp_nx[0]
    distances_nx = sssp_nx[1]
...
def test_louvain_community_detection(
    networkx_weighted_undirected_8_12, kg_from_nx_ud_8_12
):
...
def test_translation_subgraph_extraction(
    networkx_weighted_directed_8_12, kg_from_nx_di_8_12
):
    se_nx = mg.algos.subgraph.extract_subgraph(
        networkx_weighted_directed_8_12, {0, 2, 3}
    )
...
def test_labal_propagation(networkx_weighted_undirected_8_12, kg_from_nx_ud_8_12):
    cd_nx = mg.algos.clustering.label_propagation_community(
...
def test_labal_propagation_kg(katanagraph_cleaned_8_12_ud, nx_from_kg_ud_8_12):
    ...
    test for katana graph which is directly loaded rather than translated from nettworkx, also test two consecutive runs with the same source code
    """
    cd_kg1 = mg.algos.clustering.label_propagation_community(
        katanagraph_cleaned_8_12_ud
    )
    cd_kg2 = mg.algos.clustering.label_propagation_community(
        katanagraph_cleaned_8_12_ud
    )
...
def test_local_clustering_coefficient(
    ...
    lcc_nx = mg.algos.clustering.local_clustering_coefficient(
        networkx_weighted_undirected_8_12, prop_name
    )
    lcc_kg = mg.algos.clustering.local_clustering_coefficient(
        kg_from_nx_ud_8_12, prop_name
    )
...
def test_local_clustering_coefficient_kg(
    ...
    """
    prop_name = "output_prop"
    lcc_kg1 = mg.algos.clustering.local_clustering_coefficient(
        katanagraph_cleaned_8_12_ud, prop_name
    )
    lcc_kg2 = mg.algos.clustering.local_clustering_coefficient(
        katanagraph_cleaned_8_12_ud, prop_name
    )
